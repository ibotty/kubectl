...
type CreateSecretTLSOptions struct {
	...
	Cert string
	// Ca is the path to the user's ca certificate.
	Ca string
...
func NewCmdCreateSecretTLS(f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().StringVar(&o.Ca, "ca", o.Ca, i18n.T("Path to PEM encoded CA certificate."))
...
func (o *CreateSecretTLSOptions) createSecretTLS() (*corev1.Secret, error) {
	...
	if len(o.Ca) != 0 {
		secretTLS.Data["ca.crt"] = []byte(tlsCa)
	}
	if o.AppendHash {
